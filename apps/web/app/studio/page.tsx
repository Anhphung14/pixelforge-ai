...
import { ArrowRight, Check, Download, ImageIcon, Layers3, LoaderCircle, RefreshCcw, ScanFace, Sparkles, SunMedium, X, Zap, AlertCircle } from 'lucide-react';
  ...
  async function handleEnhance() {
    ...
    try {
      ...
      if (err instanceof DOMException && err.name === "AbortError") {
        ...
        setError(
          isVi
            ? "Backend GPU AI hiện chưa triển khai online (yêu cầu hạ tầng GPU máy chủ). Vui lòng xem kết quả mẫu tại Showcase."
            : "AI GPU Backend is currently offline (requires dedicated GPU server). Please view samples on the Showcase."
        );
      }
    } finally {
      ...
      <div className="relative mx-auto max-w-[1440px] px-4 py-5 sm:px-6 lg:px-10 lg:py-8">
        <nav className="mb-12 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link
              href="/"
              className="flex items-center gap-1.5 rounded-xl border border-cyan-500/30 bg-cyan-500/10 px-3.5 py-1.5 text-xs font-semibold text-cyan-300 transition hover:bg-cyan-500/20 hover:text-white hover:border-cyan-400 shadow-sm shadow-cyan-500/10"
            >
              <Sparkles size={14} className="text-cyan-400" />
              <span>{isVi ? "Xem Showcase mẫu" : "Back to Showcase"}</span>
            </Link>
            ...
            <div className="flex rounded-xl border border-white/10 bg-white/[0.035] p-1" aria-label="Language selector">
              {(['vi', 'en'] as const).map((value) => (
                <button
                  ...
                  className={`rounded-lg px-3 py-1.5 text-[11px] font-bold tracking-wider transition ${language === value ? 'bg-blue-500 text-white shadow-lg shadow-blue-500/20' : 'text-slate-500 hover:text-white'}`}>
                  {value.toUpperCase()}
                </button>
              ))}
            </div>
        ...
        {/* Notice: Structural Reference / Backend GPU offline */}
        <div className="mx-auto mb-10 max-w-4xl rounded-2xl border border-amber-500/30 bg-amber-500/10 p-4 text-xs text-amber-200">
          <div className="flex items-center gap-2 font-bold text-amber-300">
            <AlertCircle size={16} />
            <span>{isVi ? "Chế độ tham khảo cấu trúc giao diện (Backend GPU Offline)" : "Structural Reference Mode (GPU Backend Offline)"}</span>
          </div>
          <p className="mt-1.5 text-zinc-300 leading-relaxed">
            {isVi
              ? "Trang này phục vụ việc tham khảo kiến trúc giao diện điều khiển Studio. Do các mô hình AI (Real-ESRGAN, CodeFormer, Retinexformer) cần GPU máy chủ hiệu năng cao nên backend hiện chưa được triển khai online. Mời bạn ghé qua "
              : "This page demonstrates the Studio interface architecture. As AI models require high-end GPU infrastructure, live inference is currently offline. Please visit "}
            <Link href="/" className="font-bold text-cyan-400 underline hover:text-cyan-300">
              {isVi ? "Trang Showcase & Kết quả mẫu" : "the Showcase & Sample Results page"}
            </Link>
            {isVi ? " để xem các ảnh đã xử lý thực tế." : " to explore actual processed outputs."}
          </p>
        </div>
        ...
        <header className="mx-auto mb-10 max-w-3xl text-center lg:mb-14">
          ...
          <p className="mx-auto mt-5 max-w-2xl text-pretty text-sm leading-7 text-zinc-400 sm:text-base">
            {isVi
              ? 'Làm nét, phục hồi khuôn mặt, cân bằng ánh sáng và làm tươi màu ảnh bằng các mô hình AI chuyên biệt.'
              : 'Sharpen details, restore faces, balance lighting and revive colors with purpose-built AI models.'}
          </p>
        </header>
        ...
        {!original ? (
          <div className="mx-auto max-w-4xl">
            ...
            <div className="mt-5 grid grid-cols-2 gap-3 text-xs text-zinc-500 sm:grid-cols-4">
              {(isVi
                ? ['Tối đa 10 MB', 'JPG · PNG · WebP', '2× hoặc 4×', 'Không lưu ảnh']
                : ['Up to 10 MB', 'JPG · PNG · WebP', '2× or 4×', 'No image storage']
              ).map((item) => (
            ...
            <div className="studio-preview">
              <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                <div className="flex min-w-0 items-center gap-3">
                  <span className="rounded-xl border border-white/10 bg-white/5 p-2 text-blue-300">
                    <ImageIcon size={18} />
                  </span>
                  ...
                  <div className="absolute inset-0 z-30 grid place-items-center bg-[#09090e]/75 backdrop-blur-sm">
                    <div className="text-center">
                      ...
                      <button
                        ...
                        className="mt-4 text-xs text-zinc-400 underline decoration-zinc-600 underline-offset-4 hover:text-white">
                        {isVi ? 'Hủy xử lý' : 'Cancel'}
                      </button>
                    </div>
                  </div>
                )}
              ...
              {effectiveDimensions && (
                <div className="mt-4 flex flex-wrap items-center gap-2 text-xs text-zinc-500">
                  <span className="rounded-lg bg-white/5 px-2.5 py-1.5">
                    {isVi ? 'Ảnh gốc' : 'Original'} {dimensions?.width} × {dimensions?.height}
                  </span>
                  <ArrowRight size={13} />
                  ...
                  {effectiveDimensions.resized && (
                    <span className="text-cyan-300">
                      {isVi ? 'Ảnh được tối ưu về 1600 px trước khi xử lý' : 'Image is optimized to 1600 px before processing'}
                    </span>
                  )}
                </div>
              )}
            ...
            <aside className="studio-controls">
                ...
                <div className="mt-3 grid grid-cols-2 gap-2">
                  {(['general', 'anime'] as ModelType[]).map((type) => (
                    <button
                      ...
                      className={`segmented ${modelType === type ? 'segmented-active' : ''}`}>
                ...
                <div className="mt-3 grid grid-cols-2 gap-2">
                  {([2, 4] as UpscaleScale[]).map((value) => (
                    <button
                      ...
                      className={`scale-option ${scale === value ? 'scale-option-active' : ''}`}>
                      <span className="text-lg font-semibold">{value}×</span>
                      <span className="text-[10px] text-zinc-500">
                        {value === 2 ? (isVi ? 'Nhanh & nhẹ' : 'Fast & light') : isVi ? 'Chi tiết tối đa' : 'Maximum detail'}
                      </span>
                    </button>
                  ))}
                </div>
                ...
                <div className="flex flex-wrap items-end justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      {enabledEnhancements === 3 ? (isVi ? 'Tắt tất cả' : 'Clear all') : isVi ? 'Bật tất cả' : 'Enable all'}
                    </button>
                  </div>
                </div>
                <div className="mt-3 grid gap-2 sm:grid-cols-3 lg:grid-cols-1 xl:grid-cols-3">
                  <ToggleCard
                    active={freshnessEnhance}
                    disabled={loading}
                    onClick={() => setFreshnessEnhance((value) => !value)}
                    icon={<RefreshCcw size={18} className="text-cyan-300" />}
                    label={isVi ? 'Làm tươi' : 'Refresh'}
                    description={isVi ? 'Màu trong và sống động' : 'Clean, vivid colors'}
                    tone="cyan"
                  />
                  <ToggleCard
                    active={lowLightEnhance}
                    disabled={loading}
                    onClick={() => setLowLightEnhance((value) => !value)}
                    icon={<SunMedium size={18} className="text-sky-300" />}
                    label={isVi ? 'Làm sáng' : 'Relight'}
                    description={isVi ? 'Cứu chi tiết vùng tối' : 'Recover dark details'}
                    tone="sky"
                  />
                  <ToggleCard
                    active={faceEnhance}
                    disabled={loading}
                    onClick={() => setFaceEnhance((value) => !value)}
                    icon={<ScanFace size={18} className="text-blue-300" />}
                    label={isVi ? 'Khuôn mặt' : 'Face restore'}
                    description={isVi ? 'Phục hồi ngũ quan' : 'Rebuild facial details'}
                  />
                </div>
              </div>
              ...
              {(freshnessEnhance || lowLightEnhance || faceEnhance) && (
                <div className="mb-7 space-y-5 rounded-2xl border border-white/8 bg-black/20 p-4">
                  {freshnessEnhance && (
                    <RangeControl
                      label={isVi ? 'Độ tươi màu' : 'Color freshness'}
                      value={freshnessStrength}
                      onChange={setFreshnessStrength}
                      disabled={loading}
                      accent="cyan"
                    />
                  )}
                  {lowLightEnhance && (
                    <RangeControl
                      label={isVi ? 'Cường độ làm sáng' : 'Relight intensity'}
                      value={lowLightStrength}
                      onChange={setLowLightStrength}
                      disabled={loading}
                      accent="sky"
                      min={0.1}
                    />
                  ...
                  {faceEnhance && (
                    <div>
                      <div className="mb-2 flex items-center justify-between text-xs">
                        <span className="text-zinc-400">{isVi ? 'Phục hồi khuôn mặt' : 'Face restoration'}</span>
                        <span className="text-blue-300">{faceRestorer === 'codeformer' ? 'CodeFormer' : 'GFPGAN'}</span>
                      </div>
                      <div className="grid grid-cols-2 gap-2">
                        {(['codeformer', 'gfpgan'] as FaceRestorer[]).map((restorer) => (
                          <button
                            key={restorer}
                            type="button"
                            disabled={loading}
                            onClick={() => setFaceRestorer(restorer)}
                            className={`mini-segment ${faceRestorer === restorer ? 'mini-segment-active' : ''}`}>
                            {restorer === 'codeformer' ? (isVi ? 'Ảnh mờ nặng' : 'Severe blur') : isVi ? 'Tự nhiên' : 'Natural'}
                          </button>
                        ))}
                      </div>
                      {faceRestorer === 'codeformer' && (
                        <div className="mt-4">
                          <RangeControl
                            label={isVi ? 'Giữ nét nhận dạng' : 'Identity fidelity'}
                            value={fidelity}
              ...
              {error && (
                <p role="alert" className="mb-4 rounded-xl border border-red-400/20 bg-red-400/5 px-3 py-2.5 text-xs leading-5 text-red-300">
                  {error}
                </p>
              )}
              ...
              {result && (
                <a href={result} download="pixelforge-enhanced.png" className="secondary-action">
                  <Download size={18} /> {isVi ? 'Tải ảnh chất lượng cao' : 'Download high-quality image'}
                </a>
              )}
            </aside>
          </section>
        ...
        <footer className="mt-14 flex flex-col items-center justify-center gap-3 border-t border-white/5 py-8 text-center text-xs text-zinc-500">
